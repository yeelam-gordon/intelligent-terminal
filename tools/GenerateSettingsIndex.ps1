...
foreach ($xamlFile in Get-ChildItem -Path $SourceDir -Filter *.xaml)
{
    ...
    if ($ClassMap.ContainsKey(($pageClass)) -and -not (IsProfileSubPage $pageClass))
    {
        $entries += [pscustomobject]@{
            ...
            ElementName     = $null # No specific element to navigate to on the page itself
            File            = $filename
        }
    }
...
namespace winrt::Microsoft::Terminal::Settings::Editor::implementation
{
    struct IndexEntry
    {
        ...
        // SubPage to navigate to on pages with multiple subpages (i.e. Profiles, New Tab Menu)
