...
///   * `old_sid = None` (boot-time load failure with no bootstrap): surface
///     the failure and leave the tab unbound. A timeout is indeterminate:
///     issuing `session/new` could race a late successful load and create two
///     live sessions.
async fn handle_load_failure(
    old_sid: Option<&acp::schema::v1::SessionId>,
    tab_id: String,
    _cwd: std::path::PathBuf,
    _conn: conn::ClientLink,
    ...
    // Boot-time load is indeterminate until master has returned. Never
    // create a replacement session here: ACP cancellation is cooperative and
    // a late load may still become live in the agent.
    tab_to_session.lock().await.remove(&tab_id);
    let _ = event_tx.send(AppEvent::TabError {
        tab_id,
        message: error_message,
    });
...
fn dispatch_load_session(
    ...
    tokio::task::spawn_local(async move {
        ...
        // Reserve the target id before awaiting load. Prompt/autofix paths
        // must not interpret an empty map as permission to lazily create a
        // second session while this resume is in flight.
        tab_to_session
            .lock()
            .await
            .insert(req.tab_id.clone(), session_id.clone());
        let mut load_req =
