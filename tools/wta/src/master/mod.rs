...
struct AgentCli {
    ...
    /// The path namespace accepted by this agent's `session/new` and
    /// `session/load` requests. Explicit WSL sources are known up front;
    /// opaque custom launchers are learned from their own `session/list` and
    /// finally from a successful cwd-retry candidate.
    cwd_format: Mutex<CwdFormatState>,
...
impl CwdFormatState {
    fn for_source(source: &crate::agent_source::AgentSource) -> Self {
        match source {
...
impl HelperHandler {
    ...
    /// `session/new`. A rejected cwd means the session was not loaded, so it
    /// is safe to retry only that narrow class of agent error.
...
impl HelperHandler {
    ...
    async fn new_session(
        ...
        let (resp, cwd_for_registry) = self
    ...
    async fn load_session(
        ...
        let mut cwd_for_registry = args.cwd.clone();
        tracing::info!(
            target: "master",
            step = "helper→agent",
            op = "load_session",
        ...
        let resp = if is_orphan_rebind {
            ...
            match self.forward_load_session_to_agent(&agent, args).await {
                Ok((resp, effective_cwd)) => {
                    cwd_for_registry = effective_cwd;
                    resp
                }
...
async fn spawn_one_agent(
    ...
    Ok(Arc::new(AgentCli {
        ...
        cli_source,
        source: source.clone(),
        cwd_format: Mutex::new(CwdFormatState::for_source(source)),
        cmd_key: key.clone(),
    }))
}
