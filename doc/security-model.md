...
| Diagnostic logs may disclose sensitive data | Information disclosure | Medium | WTA logs may contain command lines, event payload summaries, errors, and metadata. At the helper's WT event receipt boundary, Release builds emit one `info` record with the method only; internal Debug builds instead emit one `debug` record with the method and full envelope. Separate routing diagnostics may still record metadata at `trace`. Other raw user/agent content (prompts, responses, terminal output, typed input) remains gated to `trace`. Known examples include `wta-main_*.log`, `wta-delegate.log`, `terminal-agent-pane.log`, and `wta-install-hooks.log` under `logs\<pkgver>\`. Retention is bounded (only the current version's dir kept, daily cli rotation, 3-day per-PID helper prune). |
